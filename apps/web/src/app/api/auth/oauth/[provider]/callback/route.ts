...
const DEFAULT_ROLE = process.env.CAM_DEFAULT_USER_ROLE?.trim() || 'developer';
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  if (!provider) {
    return redirectToLoginWithError('OAuth 提供商未启用', request);
  }
  ...
  if (errorParam) {
    ...
    return redirectToLoginWithError(`OAuth 错误: ${desc}`, request);
  }

  if (!code || !stateParam) {
    return redirectToLoginWithError('OAuth 回调缺少必要参数', request);
  }

  // 验证 state（从 Cookie 比对）
  const storedState = request.cookies.get('cam_oauth_state')?.value;
  if (!storedState || storedState !== stateParam) {
    return redirectToLoginWithError('OAuth state 验证失败（可能已过期，请重试）', request);
  }

  if (!verifyOAuthState(stateParam, providerId)) {
    return redirectToLoginWithError('OAuth state 签名无效', request);
  }
  ...
  try {
    ...
    if (!oauthUser.providerAccountId || !oauthUser.username) {
      return redirectToLoginWithError('无法获取 OAuth 用户信息', request);
    }
    ...
    if (existingOAuth) {
      ...
      if (user?.status === 'disabled') {
        return redirectToLoginWithError('账户已被禁用', request);
      }
    } else {
      ...
      // 生成唯一用户名（避免与已有用户冲突）
      const baseUsername = oauthUser.username.toLowerCase().replace(/[^a-z0-9_-]/g, '-');
      let finalUsername = baseUsername;
      let suffix = 1;
      while (db.select({ id: users.id }).from(users).where(eq(users.username, finalUsername)).get()) {
    ...
    response.cookies.delete('cam_oauth_state');

    return response;
  } catch (err) {
    console.error(`[OAuth] ${providerId} 回调失败:`, err);
    const msg = err instanceof Error ? err.message : 'OAuth 登录失败';
    return redirectToLoginWithError(msg, request);
  }
}

/** 重定向到登录页并附加错误信息 */
function redirectToLoginWithError(message: string, request: NextRequest): NextResponse {
